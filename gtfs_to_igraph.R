...
#I (with help from ChatGPT) made some slight changes in these lines:
  #86-88 - sort coordinate formatting
  #261-263 - calculate average edge duration
...
# convert stops back into  data frame
  #I slightly changed this code to ensure coordinates were properly preserved
  coords <- sp::coordinates(stops)
  df <- cbind(as.data.frame(coords), stops@data) %>% setDT()
  setnames(df, old = names(df)[1:2], new = c("stop_lon", "stop_lat"))
...
############ 5. Indentify links between stops   -----------------
  ...
  #Calculate average travel time on each link - Jess addition!
    avg_travel_time <- stop_times_edited[, .(avg_travel_time = mean(travel_time, na.rm=TRUE)), 
                                         by=.(stop_id, stop_id_to)]
    relations <- merge(relations, avg_travel_time, by = c("stop_id", "stop_id_to"), all.x = TRUE)
